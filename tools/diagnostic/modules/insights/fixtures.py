...
INSERT INTO products (id, tenant_id, code, name, retail_price, cost_price,
                     wholesale_price, is_active, created_at)
VALUES ({pid}, {{{{tenant_id}}}}, '{code}', '{name}', {retail}, {cost},
        {wholesale}, 1, NOW() - INTERVAL {days_old} DAY)
...
VALUES ({sale_id}, {{{{tenant_id}}}}, {store_id}, {total}, '{status}', {user_id}, {customer_id},
...
def make_product(pid, code, name, retail, cost, days_old=30, wholesale=None):
...
def make_sale(sale_id, pid, unit_price, qty=1, days_ago=5,
    ...
    return SALE_TPL.format(
        ...
        total=total, status=status, days_ago=days_ago,
