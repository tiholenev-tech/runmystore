...
function _wizPriceParse(text) {
  ...
  const hasStotinki = /(стотинки?|стот\.|цент[аи]?|cents?|копейк|пени)/i.test(raw);
  let pre = raw.replace(/\bзапетая\b/gi, ',').replace(/\bточка\b/gi, '.');
  for (const k of _BG_WORD_KEYS) pre = pre.replace(new RegExp('\\b' + k + '\\b', 'gi'), ' ' + _BG_WORD_NUMS[k] + ' ');
  pre = pre.replace(/\s+/g, ' ').trim();
  const cleaned = pre.replace(/лева?|лв\.?|евро|€|eur|euro|usd|\$|gbp|£|ron|lei|лей|стотинки?|стот\.?|цент[аи]?|cents?|пени|пенс|сантим[аи]?|копейк[аи]?|около|примерно|горе|долу|май|по|и/gi, ' ').replace(/\s+/g, ' ').trim();
