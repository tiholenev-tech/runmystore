...
**Дата последна актуализация:** 28.04.2026 (вечерна сесия + late-night корекции)  
**Автор:** Тихол + Claude (синтез от 5 AI консултации + дълга вечерна сесия за всички модули)  
...
Малък бутон ПОД 4-те. Pulse анимация (тиха, не натрапчива). Тап → отваря voice overlay (ползва общия `partials/voice-overlay.php` от V0.7.23).

**Защо ПОД, не НАД:** Insights са primary attention (loss aversion печели). 4 бутона са primary actions. AI Brain е secondary — "ако искаш нещо различно". Под = достъпен в thumb zone, но не доминира.
...
- `code` (артикулен номер) — задължително празно (всеки нов артикул нов код)
- `barcode` — задължително празно
- Снимка — задължително празна (различен продукт = различна снимка)
...
### 11.4 Дизайн-полиране — empty/loading/error states (НОВО v1.2)

**Контекст:** v1.2 има основната архитектура. Преди deploy в Phase 2 (S90) трябват:

- **Empty states** — какво вижда Пешо когато:
  - life-board insights са 0 (нов магазин, нямa данни) — препоръка: placeholder "AI се запознава с магазина. Започни като продадеш нещо."
  - products списъкът е празен (0 артикула) — препоръка: голям + бутон "Добави първи артикул" + onboarding hint
  - няма recent suppliers за поръчка — препоръка: "Кой е първият ти доставчик?" voice prompt
  - няма история на продажби в orders Detailed — препоръка: AI prompt "Започни като поръчаш нещо"
- **Loading states**:
  - OCR processing 3-5 sec — neon glow spinner + "Чета фактурата…"
  - AI generating message за поръчка — typing indicator
  - Voice transcription — pulse на mic icon
- **Error states**:
  - Voice не се разчете — "Не разбрах. Опитай пак или натисни клавиатура"
  - Network fail — yellow border + toast "Нямам интернет, записвам в тефтера" (offline mode → S88)
  - AI timeout (>10s) — fallback "Бавно е. Искаш ли да продължиш ръчно?"
- **Цветове на header-ите** per module — TBD: всички qd default или per-module q-color (delivery=q5, orders=q2)?
- **Анимации** на преходите — TBD: slide left/right, fade, или native iOS/Android default
- **Точните пиксели на X бутона** — горе-вдясно, ~44×44px hit area минимум

**Препоръка:** Отделна сесия за дизайн-полиране преди S90 deploy. Може да стане в Claude Code който прави HTML мокъпи в `mockups/states/` папка.
...
### 16.1 Нови колони (за Phase 2 S90)

```sql
-- Mode preference per user (multi-user tenant support)
ALTER TABLE users 
...
-- Variation description for semi-variation sales (sale_items)
...
### 20.3 Самопроверка срещу неoменими правила (секция 17.5)
...
- ✅ Voice-first: секции 4.5, 7.2.5, 7.4.2, 17.5
- ✅ DB колони правилни: `sale_items.variation_description` в секция 16; `sales.status='canceled'` едно L споменато в 17.5
...
**КРАЙ НА SIMPLE_MODE_BIBLE.md v1.2**
