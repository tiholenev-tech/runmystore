...
// Може ли да добавя артикули — само owner и manager
$can_add = in_array($role, ['owner', 'manager']);
// Може ли да вижда доставна цена — само owner
...
.product-list { padding: 8px 16px 20px; }
.product-card {
  background: #1a1a1a;
  border: 1px solid #2a2a2a;
  border-radius: 14px;
...
.product-card:active { border-color: #6366f1; }
...
.stock-num { font-size: 20px; font-weight: 700; color: #6366f1; line-height: 1; }
...
.meta-chip.variant { color: #6366f1; }
.meta-chip.price { color: #34d399; }
.meta-chip.cost { color: #fbbf24; }
.product-bottom {
...
/* MODAL */
.modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); z-index: 300; }
...
.btn-cancel {
  width: 100%; background: #2a2a2a; color: #999;
  border: none; border-radius: 12px;
...
.variant-hint { font-size: 13px; color: #6366f1; margin-bottom: 10px; }
.variant-textarea {
  width: 100%; background: #1a1a1a;
  border: 1px solid #3a3a3a; border-radius: 8px;
  padding: 10px; color: #e5e5e5; font-size: 14px;
...
<div class="search-wrap">
  <form method="GET" action="">
    <input type="text" name="q" class="search-input"
           placeholder="Търси по ime, баркод, код..."
...
<?php if (empty($products)): ?>
  <div class="empty">
    ...
    <div class="empty-sub"><?= $search ? 'Опитай с друго търсене' : ($can_add ? 'Натисни + Добави или кажи на AI да импортира от снимка' : 'Кажи на AI да добави артикули') ?></div>
  </div>
<?php else: ?>
  ...
  <div class="product-card" onclick="viewProduct(<?= $p['id'] ?>)">
    ...
    <div class="product-meta">
      ...
      <?php if ($can_see_cost && $p['cost_price'] > 0): ?>
        <span class="meta-chip cost">Дост: €<?= number_format((float)$p['cost_price'], 2) ?></span>
      <?php endif; ?>
      <?php if ($is_zero): ?>
  ...
  <div class="modal">
    ...
    <form method="POST" action="product-save.php">
      ...
      <div class="variant-section" id="variant-section">
        ...
        <textarea name="variants_raw" class="variant-textarea" id="variants-textarea"
                  placeholder="Напр: Червена S-3, Червена M-5, Синя M-2"></textarea>
