    ...
    if ($ajax === 'products') {
        ...
        if ($needHaving) {
            ...
            $products = DB::run("SELECT p.id,p.name,p.code,p.barcode,p.retail_price,p.cost_price,p.image_url,p.supplier_id,p.category_id,p.parent_id,p.discount_pct,p.discount_ends_at,p.min_quantity,p.unit,s.name AS supplier_name,c.name AS category_name,CAST(COALESCE(
                ...
                ELSE i.quantity END
            ,0) AS SIGNED) AS store_stock,{$dse} AS days_stale, CAST(COALESCE(CASE WHEN EXISTS(SELECT 1 FROM products ch2 WHERE ch2.parent_id=p.id AND ch2.is_active=1) THEN (SELECT SUM(si99.quantity) FROM sale_items si99 JOIN sales s99 ON s99.id=si99.sale_id JOIN products cp2 ON cp2.id=si99.product_id WHERE cp2.parent_id=p.id AND s99.store_id={$sid} AND s99.status!='canceled' AND s99.created_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)) ELSE (SELECT SUM(si99.quantity) FROM sale_items si99 JOIN sales s99 ON s99.id=si99.sale_id WHERE si99.product_id=p.id AND s99.store_id={$sid} AND s99.status!='canceled' AND s99.created_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)) END,0) AS SIGNED) AS sold_30d FROM products p LEFT JOIN suppliers s ON s.id=p.supplier_id LEFT JOIN categories c ON c.id=p.category_id LEFT JOIN inventory i ON i.product_id=p.id AND i.store_id=? WHERE {$where_sql} HAVING {$hSQL} ORDER BY {$order} LIMIT ? OFFSET ?", array_merge([$sid], $params, [$per_page, $offset]))->fetchAll(PDO::FETCH_ASSOC);
            ...
            $products = DB::run("SELECT p.id,p.name,p.code,p.barcode,p.retail_price,p.cost_price,p.image_url,p.supplier_id,p.category_id,p.parent_id,p.discount_pct,p.discount_ends_at,p.min_quantity,p.unit,s.name AS supplier_name,c.name AS category_name,CAST(COALESCE(
                ...
                THEN (SELECT SUM(ci.quantity) FROM inventory ci JOIN products cp ON cp.id=ci.product_id WHERE cp.parent_id=p.id AND ci.store_id={$sid})
                ELSE i.quantity END
            ,0) AS SIGNED) AS store_stock, CAST(COALESCE(CASE WHEN EXISTS(SELECT 1 FROM products ch2 WHERE ch2.parent_id=p.id AND ch2.is_active=1) THEN (SELECT SUM(si99.quantity) FROM sale_items si99 JOIN sales s99 ON s99.id=si99.sale_id JOIN products cp2 ON cp2.id=si99.product_id WHERE cp2.parent_id=p.id AND s99.store_id={$sid} AND s99.status!='canceled' AND s99.created_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)) ELSE (SELECT SUM(si99.quantity) FROM sale_items si99 JOIN sales s99 ON s99.id=si99.sale_id WHERE si99.product_id=p.id AND s99.store_id={$sid} AND s99.status!='canceled' AND s99.created_at>=DATE_SUB(NOW(),INTERVAL 30 DAY)) END,0) AS SIGNED) AS sold_30d FROM products p LEFT JOIN suppliers s ON s.id=p.supplier_id LEFT JOIN categories c ON c.id=p.category_id LEFT JOIN inventory i ON i.product_id=p.id AND i.store_id=? WHERE {$where_sql} ORDER BY {$order} LIMIT ? OFFSET ?", array_merge([$sid], $params, [$per_page, $offset]))->fetchAll(PDO::FETCH_ASSOC);
...
function _v4ComputeFooter(axIdx){
    ...
    if(!hasVals){
        ...
        if(nextEmptyIdx<0){
            ...
            var _nextLbl=/^(размер|size|цвят|color|вариация\s*\d+)$/i.test(nextAx.name)?('Вариация '+(nextEmptyIdx+1)):nextAx.name;b2='<button type="button" onclick="S._wizActiveTab='+nextEmptyIdx+';renderWizard()" style="flex:1;height:44px;border-radius:12px;background:linear-gradient(135deg,hsl(255 70% 52%),hsl(222 70% 42%));border:1px solid hsl(255 70% 55%);color:#fff;font-size:12px;font-weight:700;cursor:pointer;box-shadow:0 4px 14px hsl(255 70% 40% / 0.4),inset 0 1px 0 rgba(255,255,255,0.25);display:flex;align-items:center;justify-content:center;gap:5px;font-family:inherit">Избери '+esc(_nextLbl.toLowerCase())+'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>';
        }
        ftMid=b1+b2;
    }
    return ftBack+ftMid;
