...
var voiceRec=null, isRecording=false, searchResult='';
...
function toggleVoice(){
  ...
  voiceRec.onerror=function(e){stopVoice();if(e.error==='no-speech')showToast('Не чух нищо — опитай пак');else if(e.error==='not-allowed')showToast('Разреши микрофона от настройките');else showToast('Грешка: '+e.error);};
...
function doWebSearch(){
  fetch('ai-helper.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'web_search',query:state.biz+' '+state.segment+' small retailer dead stock loss monthly EU average'})})
    ...
    .then(function(d){searchResult=d.result||'';})
    .catch(function(){searchResult='';});
}
...
async function showWowMoment(){
  ...
  var waited=0;
  while(!searchResult && waited<5000){ await wait(300); waited+=300; }
  searchInd.classList.remove('show'); hideTyping();

  try{
    showTyping();
    var r=await fetch('ai-helper.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'wow',prompt:buildWowPrompt()})});
