...
(function(window) {
  ...
  // For D520 SPP write: bytes → JS string with each char = byte value (Latin-1
  // mapping). The @e-is plugin is patched (S97 — see mobile/patches/) to use
  // ISO-8859-1 instead of UTF-8 server-side, so any byte 0x00-0xFF survives
  // the bridge intact. This unlocks BITMAP raster (cyrillic/€/лв) and AIMO
  // frame wrapping over SPP.
  ...
  // Alias with the name that better describes post-patch behaviour.
  const bytesToLatin1String = asciiBytesToString;
  ...
  // S97.LAYOUT — Bulgaria EUR transition: dual EUR/BGN display required by
  // Закона за Българския лев until 2026-08-08. After that, EUR-only label is OK.
  // Hardcoded cutoff so the printer self-flips at the date without a server push.
  function defaultLabelMode() {
    const cutoff = Date.UTC(2026, 7, 9, 0, 0, 0);  // 0-indexed month: 7=August → 09 Aug 00:00 UTC
    return Date.now() < cutoff ? 'dual' : 'eur';
  }

  // S97.LAYOUT — char-width-aware truncation for D520 (fixed-width bitmap fonts).
  // TSPL bitmap fonts:  "1"=8×12 (8 dots/char),  "2"=12×20 (12 dots/char),
  //                     "3"=16×24 (16 dots/char), "4"=24×32 (24 dots/char).
  function fitTextDots(text, charWidth, maxDots) {
    const max = Math.max(1, Math.floor(maxDots / charWidth));
  ...
  function generateTSPL_D520(product, store, copies, opts) {
    ...
    // S103D.FINAL — pure ASCII transliteration (was S98.PathD CP1251 — broke
    // due to plugin UTF-8 double-encoding wrecking high bytes; gave hieroglyphs
    // in bottom-right). Until APK is rebuilt with verified ISO-8859-1 patch,
    // tsplSafe() transliterates cyrillic → latin and drops anything non-ASCII.
    ...
    push('SPEED 4\r\n');
    push('CLS\r\n');
    // S103D.FINAL — CODEPAGE 1251 dropped (was for CP1251 byte payload, which
    // is now ASCII-only after tsplSafe transliteration above).
  ...
  function renderTextAsBars(text, offsetX, offsetY, fontPx, maxWidth, opts) {
    opts = opts || {};
    const minRun = opts.minRun || 1;     // S103D — was 2; smaller runs preserve detail
    const stripeH = opts.stripeH || 1;   // S103D — was 2; full vertical detail (smoother glyphs)
    const fontSpec = opts.fontSpec || ('900 ' + fontPx + 'px Arial,sans-serif');  // S103D — heavier weight (was bold)
    ...
    const px = ctx.getImageData(0, 0, w, h).data;
    const cmds = [];
    // S103D — fix: respect align by shifting canvas's "left edge" in printer
    // coordinates. Caller passes offsetX as anchor point (left/center/right edge).
    const startX = (align === 'right')  ? (offsetX - w) :
                   (align === 'center') ? (offsetX - Math.floor(w / 2)) :
                                          offsetX;
    ...
    for (let y = 0; y < h; y += stripeH) {
      ...
      while (x < w) {
        ...
        const runW = x - start;
        if (runW >= minRun) {
          cmds.push('BAR ' + (startX + start) + ',' + (offsetY + y) + ',' + runW + ',' + sH + '\r\n');
        }
      }
    }
  ...
  function generateTSPL_D520_hybrid(product, store, copies, opts) {
    ...
    if (mode === 'noprice') {
      ...
      if (color) {
        placeText(color, cursorX, 95, 18, W - cursorX - PAD);
      }
    }

    // ── SEP2 + PRICE (y 118-186) ──────────────────────────────────────
    if (mode !== 'noprice') {
      push('BAR ' + PAD + ',118,' + (W - 2 * PAD) + ',1\r\n');
    }
    if (mode === 'eur') {
      placeText(priceEur(amt), W / 2, 122, 44, W - 2 * PAD, { align: 'center' });
    } else if (mode === 'dual') {
      placeText(priceEur(amt), W / 2, 122, 44, W - 2 * PAD, { align: 'center' });
      placeText(priceBgn(amt), W / 2, 168, 20, W - 2 * PAD, { align: 'center' });
    }

    // ── BOT: stacked importer + origin (y 192-228, fits in label) ─────
    const originParts = [];
    if (matOrigin) originParts.push(matOrigin);
    if (country)   originParts.push('Произход: ' + country);
    const originStr = originParts.join(' · ');
    const importStr = importer
      ? ('Внос: ' + importer + (impCity ? ', ' + impCity : ''))
      : '';
    if (originStr || importStr) {
      push('BAR ' + PAD + ',192,' + (W - 2 * PAD) + ',1\r\n');
      // S103D.D — extra left margin (PAD+8=16) for bottom rows; printer
      // feed alignment crops first ~5 dots on left edge of label.
      const BOT_PAD = PAD + 8;
      let rowY = 196;
      if (importStr) { placeText(importStr, BOT_PAD, rowY, 12, W - 2 * BOT_PAD); rowY += 14; }
      if (originStr) { placeText(originStr, BOT_PAD, rowY, 12, W - 2 * BOT_PAD); }
    }
  ...
  const CapPrinter = {
    ...
    async _printD520(product, store, copies, opts) {
      ...
      // S103D.HYBRID — switched to 'hybrid': cyrillic rendered as TSPL BAR
      // primitives (pure ASCII wire; plugin UTF-8 wrapping cannot corrupt).
      // Each cyrillic field rendered to canvas, RLE'd into BAR x,y,w,h
