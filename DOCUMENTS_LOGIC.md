...
**Версия:** v1.0
**Дата:** 09.05.2026
**Статус:** ДРАФТ — pre-implementation review needed
...
## 12. OPEN QUESTIONS (за решаване с Тихол преди implementation)

1. **СУПТО декларация:** ENI ще се декларира ли като СУПТО през НАП? Това отключва различни legal изисквания (QR код в касов бон, ежедневно подаване на данни).
2. **Stripe Connect интеграция:** Платежният статус се обновява ли автоматично при Stripe webhook или manual?
3. **Гаранционна карта default:** За всеки product с has_warranty=1 ли се печата автоматично, или само при Layer 1 бутон „ГАРАНЦИЯ"?
4. **Multi-tenant tenants.country_code:** Ако има non-BG tenant — кои документи са applicable? (cash_order и proforma са универсални; protocol_117 е БГ-specific)
5. **Storage backup:** PDF-ите ходят ли в backup стратегията или само DB? (recommended: и двете, но storage расте)
6. **Series end_number дефолт:** 999_999_999 (start + 999M) ли е sane дефолт, или по-малко?
7. **Anti-fraud:** Протокол при подозрителни смени на серии (например owner намалява current_number)? Telegram alert?

---

**END OF DOCUMENTS_LOGIC.md v1.0**
