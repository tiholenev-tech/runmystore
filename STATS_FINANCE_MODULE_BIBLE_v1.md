...
> *"s82-dash е малко копие на RunMyWallet home — само финансова карта в life-board.php."*
...
❌ Поръчай бутон [📋]                     ✅ Tap → wallet/analysis.php
❌ 4 ротиращи слота                       ✅ Sacred Glass еди карта
```

## 24.2 Защо корекцията

s82-dash е **финансовият огледален двойник** на wallet/home.php:
...
─────────────────────────────────────
   ...
   │                      │
   ▼                      ▼
wallet/home.php          life-board.php
(RunMyWallet full)     (RunMyStore home)
   │                      │
   │                  ┌───┴────┐
...
function s82VoiceRecord() {
    // Отваря voice overlay (same component като wallet/home.php)
    openVoiceOverlay({
        callback: (parsed) => {
...
function s82OpenAnalysis() {
    // Tap → пълно analysis табло (като wallet/analysis.php)
    // За RMS user отваря: stats.php?tab=finance
    // За CFO user отваря: wallet/analysis.php
...
| CFO user (plan=wallet) | N/A (има wallet/home.php) |
...
| CFO €4.99 | N/A (wallet/home.php е equivalent) |
...
═══════════════════════════════════════════════════════════════
# ETAP 5 — RUNMYWALLET + RUNMYSTORE DUAL-PRODUCT ARCHITECTURE
...
**RunMyWallet** е personal finance tracker за свободни професии. **RunMyStore.AI** е retail management система. И двата продукта **споделят 80% от codebase-а** — един engine, два UI shell-а.
...
└──────────────────────────────────────────────────────────┘
   ...
   │ tenants.plan='wallet'  │         │ start/pro/business│
...
**Upgrade path е безопасен** — всички данни остават, само се отключват нови modules. RunMyWallet user отваря магазин → upgrade на START → запазва историята си.
...
if ($tenant->plan === 'cfo') {
    // RunMyWallet shell
    header('Location: wallet/home.php');
} else {
    // RunMyStore shell
    header('Location: life-board.php');
}
...
Това което RunMyWallet **директно ползва** без промяна:

| RunMyStore компонент | RunMyWallet ползва | Промяна |
|---|---|---|
...
| Plan-based gating UI | ✅ 100% | Нов plan='wallet' |
...
RUNMYWALLET ще ползва директно:
...
landing.runmymoney.bg или landing.runmystore.bg
    ...
    ┌─────────────────────────────────┐
    │ ● Личните си финанси            │  → plan='wallet'
...
## 29.2 RUNMYWALLET Onboarding (3 minutes)
...
Сценарий: RunMyWallet user реши да отвори физически магазин.
...
Същата структура от RunMyStore Bible §6.2 M-008. RunMyWallet просто я ползва без промяна.
...
ВСЕКИ AI insight в RunMyWallet следва Закон №2 от RunMyStore: **числата идват от PHP/SQL, AI само форматира текста**. Това гарантира zero halucination.
...
## 32.3 Каталог: 30 AI ТЕМИ ЗА RUNMYWALLET
...
- **RunMyWallet:** като главна страница "Анализ"
...
|---|---|---|
| RunMyWallet user | ✅ Main page | ✅ Default mode |
...
**RunMyWallet change за GDPR compliance:**
```
...
**Защо:** RunMyStore Web Speech API е valid за price-only (numbers без personal context). RunMyWallet записва free-form personal info (имена, адреси, лични харчове) — задължително zero-data retention.
...
**File:** `wallet/onboarding.php` (нов) extends shell от `onboarding.php`.

### Bottom navigation

**RunMyStore:** [AI] [Склад] [Финанси] [Продажба] [Магазини*]

**RunMyWallet:** [AI] [Записи] [Анализ] [Цели]
...
- 30 prompt templates в `lib/ai-prompts/wallet/`
...
NEW CODE:
  ...
  ai-prompts/wallet/* (30 templates):         ~1 500 редa
...
- RunMyWallet startup → < 10K за първите 6-12 месеца.
...
- **§28** — Dual-product architecture (RunMyWallet + RunMyStore)
...
═══════════════════════════════════════════════════════════════
# ETAP 6 — RUNMYWALLET v1.2: SELF-LEARNING + ENRICHMENTS
...
RunMyWallet **намалява AI разходите** чрез непрекъснато учене. Първите 2 месеца — Gemini парсва 90% от voice inputs. След 6 месеца — PHP парсва 75% от inputs **без AI call**. 
...
RunMyWallet и RunMyStore **се хранят един друг** със знание:

```
RUNMYWALLET data → enrich RunMyStore:
...
RunMyStore data → enrich RunMyWallet:
...
Прилагат се за **ВЕКИ AI output** в RunMyWallet:
...
User-и които ползват счетоводен софтуер (Sigma, Microinvest, Ajur) могат да **подават точни числа** в RunMyWallet — voice или photo. Системата калибрира приблизителните оценки с реалните данни.
...
**RunMyWallet попълва тази дупка.** Не дублираме счетоводителя — допълваме го.
...
Това е достатъчно за **пълна калибрация** на всички оценки в RunMyWallet.
...
v1.1 (RunMyWallet):    8 552 реда (+3 181)
v1.2 (this update):  ~12 000 реда (+~3 500)
```

## 41.3 Финалeн scope на RunMyWallet
...
- wallet/analysis.php
- wallet/goals.php
- wallet/onboarding.php (със hidden costs interview)
- wallet/settings.php
...
**RunMyWallet е готов за имплементация.**
...
**Един код се пише веднъж. Работи и за RunMyStore, и за RunMyWallet.**
...
Семантика: RunMyWallet НЕ ги изпълнява. Plan='wallet' user не вижда тези страници.

TYPE 3: SAMO ZA RUNMYWALLET
─────────────────────────────────────────────────────
Папка: /wallet/
Примери:
  - wallet/home.php
  ...
  - wallet/analysis.php
  - wallet/goals.php
  - wallet/onboarding.php (CFO version)
  - wallet/settings.php
  - wallet/api/voice-parse.php
  - wallet/api/photo-parse.php
  - wallet/partials/home-voice-bar.php
  - wallet/partials/analysis-charts.php
...
if ($tenant->plan === 'cfo') {
    header('Location: wallet/home.php');     // → RunMyWallet UI
} else {
...
═══════════════════════════════════
  ...
  → И RunMyStore, и RunMyWallet получават подобрението
  → Един commit, два продукта upgrade-нати

Сценарий 2: Нов UI компонент САМО за CFO
═══════════════════════════════════════
  → Edit wallet/home.php (или нов wallet/partials/*.php)
...
tenant_id=8  →  plan='wallet'     →  RunMyWallet test profile (new)
```
...
Ако строим RunMyWallet като отделен product:    ~120 000 редa нов код
...
**RunMyWallet killer feature: НАП-anxiety reduction.**

Самонаетите в БГ имат хроничен страх от НАП — кога да подадат декларация, колко да платят, какво е НПР, кога преминаваш ДДС прага. RunMyWallet решава това.
...
RunMyWallet ги решава всичките четири.

## 45.2 НАП Dashboard widget

Нова секция в RunMyWallet settings → "НАП Контрол":
...
В януари-март, RunMyWallet предлага:

```
"Време е за годишна декларация по чл. 50.

Според записите в RunMyWallet за 2025:
✓ Оборот: €27 400
...
## 47.3 Финален scope на RunMyWallet
...
═══════════════════════════════════════════════════════════════
# ETAP 8 — REBRAND TO RUNMYWALLET (v1.4)
# Дата: 17.05.2026
═══════════════════════════════════════════════════════════════

# §48. BRAND ARCHITECTURE — RUNMYWALLET

## 48.1 Решение

Името "Pocket CFO" е **deprecated**. Финалното име е **RunMyWallet**.

Причини:
- Brand consistency с RunMyStore.AI (parent brand)
- "Run my [X]" формула за бъдещи продукти
- Един domain pattern (runmystore.ai → runmywallet.ai)
- Cross-promo естествен между двата продукта
- Endorsed brand стратегия (като LinkedIn под Microsoft)

## 48.2 Brand hierarchy

```
RunMyStore.AI                    ← Parent company / brand
    │
    ├── RunMyStore               ← Retail SaaS (магазини)
    │   Plans: FREE / START / PRO / BUSINESS
    │
    └── RunMyWallet              ← Personal finance (самонаети)
        Plans: FREE / START
        Tagline: "by RunMyStore.AI"
```

## 48.3 Plan & pricing структура

### RunMyWallet plans:

| Plan | Цена | За кого |
|---|---|---|
| FREE | €0/мес | Базово tracking, 50 записа/мес limit |
| START | €4.99/мес или €34.99/год | Unlimited + AI insights + анализ |

### Cross-product upgrades:

```
RunMyWallet START user        →  Може да upgrade-не към RunMyStore PRO
                                  при поява на retail бизнес.
                                  
RunMyStore PRO/BUSINESS user  →  Включва RunMyWallet START безплатно
                                  за personal финанси.
```

## 48.4 Visual brand elements

### Logo lockup:

```
┌─────────────────────────┐
│  RUNMYWALLET            │  ← Main wordmark
│  by RunMyStore.AI       │  ← Endorsement tagline (smaller, 60% opacity)
└─────────────────────────┘
```

### Header в приложението:
...
- Same brand gradient като RunMyStore (hue1 indigo → hue2 blue → hue3 cyan)
- Same fonts: Montserrat (text) + DM Mono (numbers)
- Same Sacred Glass canon
- Same Aurora background

**RunMyWallet НЕ е визуално различен от RunMyStore.** Той е същата визуална система с различен product focus.

## 48.5 Tagline хиерархия

```
Primary tagline (за external marketing):
   "Гласов CFO в джоба ти"
   "Voice CFO in your pocket"

Endorsement (винаги под logo):
   "by RunMyStore.AI"

App Store kratko opisanie:
   "Voice-first finance tracker за самонаети в БГ.
    Запиши приходи и разходи с глас или снимка."
```

## 48.6 Domain & socials

```
DOMAINS (за регистрация):
- runmywallet.ai          (primary)
- runmywallet.com         (redirect)
- runmywallet.app         (App Store landing)
- runmywallet.bg          (Bulgarian market)
...
SOCIALS:
- @runmywallet (Twitter, Instagram, TikTok)
- /runmywallet (LinkedIn, Facebook)
```

## 48.7 DB schema implications

```sql
-- tenants.plan ENUM update:
ALTER TABLE tenants 
  MODIFY plan ENUM(
    'free',
    'start',     -- RunMyStore START €19 или RunMyWallet START €4.99
    'pro',       -- RunMyStore PRO €49
    'business',  -- RunMyStore BUSINESS €109
    'wallet'     -- RunMyWallet FREE (basic)
  ) NOT NULL DEFAULT 'free';

-- Допълнителен ENUM за product:
ALTER TABLE tenants
  ADD COLUMN product ENUM('store', 'wallet') NOT NULL DEFAULT 'store';

-- Routing:
IF tenants.product = 'wallet' AND tenants.plan IN ('wallet','start') 
   → wallet/home.php
IF tenants.product = 'store'
   → life-board.php (RunMyStore)
```

## 48.8 Migration plan

```
ФАЗА A (S148-S150): Code paths
   - cfo/* → wallet/*
   - All file references updated
   - tenant_id=8 = wallet test profile

ФАЗА B (S150): UI rebranding
   - Header brand: POCKETCFO → RUNMYWALLET
   - Plan badge: CFO → START
   - All copy: "CFO" → "RunMyWallet"
   - Mockups updated
...
ФАЗА D (Post-launch):
   - Cross-promo banners в RunMyStore
   - "Имаш магазин? Виж RunMyStore" в RunMyWallet
   - Bundled plans
```

## 48.9 AI topic IDs — НЕ се променят

```
cfo_001 → cfo_060   ← Запазват се като DB constants
                       (не са user-facing)

В UI се показват като "AI Insight" или специфичен label,
никога не се показва "cfo_046" към потребителя.
```

## 48.10 RBAC update

```sql
-- За RunMyWallet users:
INSERT INTO roles VALUES ('wallet_user', 'RunMyWallet potrebítel');

-- Permissions:
- wallet/home.php       ✓ Pristup
- wallet/records.php    ✓ Pristup
- wallet/analysis.php   ✓ Pristup
- wallet/goals.php      ✓ Pristup
- products.php          ✗ Не виждат (RunMyStore-only)
- sale.php              ✗ Не виждат
- life-board.php        ✗ Не виждат
```

## 48.11 Mockup naming

```
mockups/P20_runmywallet_home.html           ← Replaces P20_cfo_home_v3
mockups/P20b_runmywallet_onboarding.html    ← Replaces P20b_cfo_onboarding
mockups/P21_dash82_v2.html                  ← Запазва се (s82-dash в life-board)
mockups/P22_runmywallet_voice_overlay.html  ← New
mockups/P23_runmywallet_records.html        ← New
mockups/P24_runmywallet_analysis.html       ← New
mockups/P25_runmywallet_goals.html          ← New
