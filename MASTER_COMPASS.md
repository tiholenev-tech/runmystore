...
**Code Code design migration (FAILED 3 пъти):**
- S104+S105 (Code 1) + S106 (Code 2) — 21 модула shallow migration → counter-test показа visual regression → REVERTED commit `5f36cbd`
- S113 (Code 1) — products.php rewrite (16K LOC) → "съшит вариант между стария и новия" → REVERTED commits `072520f` + `5220d1e`. 3-ти disaster.

**Audit Code Code sessions (read-only, /tmp/):**
- S114.AIBRAIN_AUDIT (30min) — pill class-name mismatch bug, ai_brain_queue table exists no producers
- S115.PERF_AUDIT (45min) — 10 P0 slow queries (correlated subqueries 300-1500ms)
- S116.SECURITY_AUDIT (50min) — CRITICAL dev-exec.php RCE, HIGH 11/12 CSRF, HIGH session fixation
- S117.I18N_AUDIT (25min) — t() не съществува, 0% non-BG launch readiness

**Audit fixes (drafts + 2 applied):**
- S116 dev-exec.php QUARANTINED ⚡ commit `91b04c4`
- S119 session_regenerate + cookie flags ⚡ commit `50a7451` (verified Set-Cookie: secure HttpOnly SameSite=Strict)
- S118 CSRF (11 patches) — drafts в /tmp/csrf_fix/, apply ден 7
- S120 PERF (4 patches + INDEXES.sql) — drafts в /tmp/perf_fix/, ~600-1500ms peeled, apply ден 7

**Mockup design phase (Опус):**
- 12 mockups APPROVED (~17K lines): P2-P6, P10 (life-board REPLACE), P11 (chat REPLACE)
- 5 нови docs: HANDOFF_CONSOLIDATED + HANDOFF_P4_P4b_FIND_COPY + DETAILED_MODE_DECISION + SESSION_HANDOFF_CONSOLIDATED + AI_STUDIO_LOGIC_DELTA
- Pending: P1 (empty state), P7-P9, P8b ×5, P8c

**Decisions FINAL (do not re-solve):**
1. products.php = FORBIDDEN ZONE за Code Code rewrite (3 disasters proven) → Standing Rule #31
2. Mockup = ground truth, DELETE+INSERT not MERGE → Standing Rule #32
3. Sequential pilot pattern (1 file/session) → Standing Rule #33
4. Iron Law gate v2 (6 layers) — design-kit/iron-law.sh TODO → Standing Rule #34

**Sequential rewrite plan:**
- Day 1 (09.05): life-board.php pilot (P10) — proof of concept
- Day 2: chat.php (P11)
- Day 3: ai-studio.php + 2 partials (P8/P8b/P8c)
- Day 4-6: products.php manual assembly (P2-P9) — РЪЧНО, не Code Code
- Day 7 (13.05): audit batch apply (CSRF + perf + AIBRAIN pill)
- 14-15.05: BETA LAUNCH ENI 🚀

**REWORK QUEUE updates:**
- Closed: DESIGN-FRANKENSTEIN, DEV-EXEC-RCE, SESSION-FIXATION, INSECURE-COOKIES, S110.LOGS-CRON-CHROMIUM
- New #82-89: Iron Law impl, products manual assembly, audit batch apply, API key rotation, AI Brain Phase 2, i18n framework, dev-exec retention, kernel reboot

**Lessons:**
- Code Code не може rewrite-ва monoliths без regression (3× proven). DELETE+INSERT prompt + малки files only.
- `git add -A` опасно — почти push-нахме 196K backup lines на public repo.
- Pre-commit hook не помага ако се relax-ва (Code 2 направи threshold 5→10 за shallow migration).
- Audit reports в /tmp/ са ETHEMERAL — copy в handoffs/audits/ преди reboot.

**Latest commits:** dd23855, 0b636db, 447c74b, 50a7451, 4c4921d, 4eebfe8, 91b04c4, 5f36cbd, 1804b8c.

Detailed handoff: `SESSION_HANDOFF_FOR_NEXT_SHEF.md` (committed 16d6535).
...
- **#32 — Mockup = ground truth, DELETE+INSERT not MERGE (08.05.2026)**
  ...
  - INSERT mockup 1:1 (body innerHTML, не header/footer-те които идват от partials/)
  - НИКАКВА интерпретация, оптимизация, "preserve old"
  - Prompt-ът трябва EXPLICIT: "ZERO MERGE. ZERO PRESERVATION."

- **#33 — Sequential pilot pattern за visual rewrite (08.05.2026)**
  Никога не пускай 2+ Code Code сесии паралелно за visual rewrite на различни modules.
...
## S143 — 13.05.2026 — РЕШЕНИЯ ОТ КОНСУЛТАЦИЯ С ТИХОЛ

Шеф-чат сесия #143. Чат с Тихол за продуктовия модул редизайн + AI стратегия.

### Свършено в S143:

1. **products-v2.php v1**: AJAX search + filter_options endpoints. CSS + HTML за search dropdown и filter drawer.

2. **products-v2.php v2**: Sticky search (input лепи горе при писане), акордеон filter drawer, ново подреждане, cascading filters.

3. **products-v2.php v3**: ОБЩА КАРТИНА — store_id=0 поддръжка във всички 12 SQL queries + AJAX endpoints.

4. **DB миграция (20260513_001)**: 4 нови колони — `products.gender` (ENUM), `products.season` (ENUM), `products.brand` (VARCHAR), `inventory.last_counted_at` (DATETIME) + 3 индекса.

5. **CORE_BUSINESS_RULES.md** създаден — централно място за бизнес правила.

6. **TOMORROW_WIZARD_REDESIGN.md** — детайлен план за S144.

### КЛЮЧОВИ РЕШЕНИЯ ВЗЕТИ В S143 (НЕЗАМЕНИМИ):

#### A. Категориите са 2 вида (записано в CORE_BUSINESS_RULES.md)

**Глобална категория** (без избран доставчик) = всички продукти от тази категория, всеки доставчик.
**Категория по доставчик** (със избран доставчик) = само продуктите на този доставчик в тази категория.

Cascading в UI: избор на доставчик → reload категории (само тези с продукти от доставчика).
SQL: вече работи естествено с `WHERE category_id=? AND supplier_id=?`.

#### B. Обща картина = default за продукти

Тих: "Това е огромен фундаментален пропуск". store_id=0 = всички магазини, sumira всички KPI.
Default за нова сесия = 0 (всички). Опцията "🏢 Всички магазини" е най-отгоре в dropdown-а.

#### C. AI стратегия: 2 описания (КРАТКО + ДЪЛГО)

**Кратко описание** (нова колона нужна или ползва съществуваща `description`):
- 20-50 думи
- Включва: принт, стил, дължина, повод, силует, материал, всичко "меко"
- AI генерира **БЕЗПЛАТНО** заедно с категория/цвят/др. — в едно AI обаждане при добавяне на снимка
- Ползва се за FULLTEXT търсене (вътрешно)

**Дълго описание** (за Google/Ecwid онлайн магазин):
- 100-200 думи
- Маркетингов език
- AI генерира **ПЛАТЕНО** — отделно AI обаждане, по натискане на бутон
- Включено в по-горни планове или по натискане

#### D. Какво НЕ правим (отказани идеи)

❌ **Нови ENUM колони за принт/стил/възраст/дължина** — заместват се от краткото описание.
❌ **AI автоматично създаване на категории** — НИКОГА. AI само предлага от съществуващите. Само owner може да създава нови категории в settings.

#### E. AI обучение — 4 нива стратегия (РЕШЕНИЕ КЪМ 13.05.2026):

| Ниво | Какво е | Кога | Точност | Цена |
|---|---|---|---|---|
| 1 | Баркод lookup в база (точно копие) | **С wizard редизайн (S144)** | 100% | €0 |
| 2 | Perceptual hash на снимки (подобни → копираме) | **С wizard редизайн (S144)** | 95% | €0 |
| 3 | Local rule engine от потвържденията на Пешо | **След 2-3 месеца** (когато има данни) | 88% | €0 |
| 4 | Локален ML модел (GPU инфраструктура) | **Чак при 50+ магазина** | 85-90% | -€150/мес инфраструктура |

**Логика на решението:**
- Ниво 4 (локален модел) не се отплаща при <50 магазина — инфраструктурата (€150/мес) надделява над AI спестяванията.
- Ниво 1+2 = ~12 часа работа, спестяват ~50% AI разходи.
- Ниво 3 изисква 1000+ потвърдени артикули = месеци време.
- Локалните модели имат 70-85% точност vs Gemini 90-95% — губим precision за пари.

**Цитат на Тих:** "ОК САМО ЗАПИШИ ВСИЧКО В КОМПАСА"

#### F. Snapshot за бъдещо обучение (НОВО ПРАВИЛО)

Когато AI обработи снимка → задължително записваме snapshot в DB (нова таблица `ai_snapshots`):
- Перceptual hash на снимката
- Връщане от AI (категория, цвят, описание, etc.)
- Потвърдено от Пешо ДА/НЕ
- Финална стойност записана от Пешо

Това става **подготовка за Ниво 2 и Ниво 3**, които ще се ползват по-късно.

### Standing Rules — нови:

- **Rule #38**: AI никога не създава категории сам — само предлага от съществуващите.
- **Rule #39**: Всеки AI обаждане с снимка → snapshot в ai_snapshots таблица (за бъдещо обучение).
- **Rule #40**: При <50 магазина — не правим локален ML модел. Само Gemini + cache + rules.
- **Rule #41**: Категориите са 2 вида — глобални (без доставчик) и по доставчик (с доставчик). UI cascading-ва.
- **Rule #42**: Default store_id за нова сесия = 0 (всички магазини). Опцията "🏢 Всички магазини" е винаги най-отгоре.

### Pending за S144 (УТРЕ):

- Редизайн на "Добави артикул" wizard
- Снимка се мести напред (на стъпка 2)
- Ниво 1 имплементация — баркод lookup
- Ниво 2 имплементация — perceptual hash + ai_snapshots таблица
- Кратко описание колона + AI prompt
- AI предлага категория с потвърждение "Закачам към X?"

### Pending за S145+ (post-wizard):

- AI snapshot accumulation (1-3 месеца)
- Ниво 3: rule engine от потвържденията
- Доставки модул (0% готов)
- Трансфери модул (0% готов)

---

## S143 — ДЕЕП РЕСЕРЧ РЕЗУЛТАТИ ЗА AI AUTOFILL (14.05.2026)

Тих пусна Gemini Deep Research проучване. Резултат: 413-редов изчерпателен
доклад "Стратегически и технологичен анализ: AI auto-fill в RunMyStore.AI".

**Файл:** `docs/AI_AUTOFILL_RESEARCH_2026.md` (копие в repo-то).

### КРИТИЧНИ РЕШЕНИЯ ВЗЕТИ С ПОДКРЕПА НА ПРОУЧВАНЕТО:

#### 1. AI Auto-fill = ДА, имплементираме (Rule #43)

**Икономическа основа:**
- Разход на снимка: $0.0015 (Gemini 2.5 Flash)
- Месечно за магазин (100 артикула): €0.15
- Спрямо €19 START план: 0.8% от прихода

**Това е блок прерязващ конкурентно предимство.** Никой в €19-€49
диапазон не предлага AI auto-fill. Loyverse, MyPOS, Smartbill — нямат AI.

#### 2. AI Модели — финален избор (Rule #44):
...
| Text gen (сигнали/описания) | **GPT-4o mini** | $0.0006/1K tok |
| Маркетинг снимки | **DALL-E 3** | $0.04/снимка |

Алтернативи за оптимизация: Qwen 2.5-VL 72B чрез DeepInfra ($0.0006/снимка).

#### 3. Лимити по план (Rule #45):

| План | Auto-fill | STT минути | SEO descr | Img gen |
|---|---|---|---|---|
| FREE €0 | 20/мес | 0 | 0 | 0 |
...
Над лимитите → кредитна система (€5 за 50 снимки или 500 STT мин).

#### 4. Оптимизации задължителни (Rule #46):

- **Prompt Caching** — Gemini/Claude поддържат, 90% намаление input cost
- **Perceptual Hashing** — €0 при повторни снимки (S144 имплементация)
- **Batch API** — 50% намаление за асинхронни задачи

#### 5. Финансов модел (Rule #47):

AI COGS = постоянни ~12.8% от приходите при добра оптимизация.

| Клиенти | MRR | AI разход | Печалба |
|---|---|---|---|
| 10 | €350 | €45 | €305 |
| 100 | €3,500 | €450 | €3,050 |
| 500 | €17,500 | €2,250 | €15,250 |
| 2,000 | €70,000 | €9,000 | €61,000 |

**Break-even: 45-50 платени клиенти.**

#### 6. Регулаторно (Rule #48):

- EU AI Act: класификация "Limited Risk" — само задължение за прозрачност
- ДДС: reverse charge mechanism за чужди AI услуги (стандартно)
- GDPR: DPA + EU data residency задължителни

### ОТКАЗАНИ ПОДХОДИ (от проучването):

❌ **Локални GPU модели** (Qwen self-hosted) — break-even само при 11+ млрд токена/мес
❌ **Claude Sonnet 4.6 за всичко** — €0.0105/снимка прекалено скъпо
❌ **GPT-5.4 Standard за масови задачи** — €0.01/снимка нерентабилно
❌ **Midjourney API** — няма pay-per-image, само абонамент

### ТРЯБВА ДА СЕ ТЕСТВА ПРЕДИ БЕТА (Critical Tests):

1. **Fuzzy Matching без халюцинации** — AI да не създава "Лятна рокля" ако
   има само "Рокли" в каталога
2. **STT с акценти и шум** — Deepgram + БГ диалекти + музика в обекта
3. **OCR на кирилица и румънски** — размазани етикети, лоша светлина

### NEW STANDING RULES:

- **Rule #43**: AI Auto-fill ВИНАГИ е включен в PRO+ план. Това е USP.
- **Rule #44**: Default AI provider = Gemini 2.5 Flash (image) + GPT-4o mini (text).
- **Rule #45**: Лимити по план задължителни, отвъд = кредити.
- **Rule #46**: Prompt caching + perceptual hashing задължителни от ден 1.
- **Rule #47**: AI COGS никога не надвишава 15% от приходите. Алармено
  при 20%, паника при 25%.
- **Rule #48**: EU AI Act прозрачност — UI винаги показва "генерирано от AI"
  за съдържание (описания, изображения).

### КОНКУРЕНТНА ПОЗИЦИЯ:

**Пазарната дупка е огромна.** В диапазона €19-€49 НИКОЙ не предлага AI:

| Конкурент | Цена | AI |
|---|---|---|
| Loyverse (силен БГ) | €0-€25 | НЯМА |
| MyPOS (БГ) | €0 | НЯМА |
| Smartbill (RO) | €5-€22 | НЯМА |
| Square | €45-€60 | Базов |
| Shopify POS | €128 | Само онлайн |
| **RunMyStore.AI** | **€19-€109** | **ПЪЛЕН** |

### НАСТРОЙКА НА S144 ПРИОРИТЕТИ:

Проучването подкрепя оригиналния план:
1. ✅ Снимка напред (стъпка 2) — задължително
2. ✅ AI auto-fill — оптимизирано за Gemini 2.5 Flash
3. ✅ Perceptual hashing (Ниво 2) — задължително
4. ✅ Baрcoode lookup (Ниво 1) — задължително
5. ✅ Кратко описание в едно AI обаждане — оптимално

**ОТКАЗ ОТ ОТЛАГАНЕТО:** Проучването показва че €0.15/мес е безопасно
дори за бета — продължаваме S144 без чакане за повече данни.

---


## S143 ИЗПЪЛНЕНО — продуктов модул редизайн (14.05.2026 EOD)

### Свършено в шеф-чат:

1. **products-v2.php v1**: AJAX search + filter_options endpoints, CSS, HTML
2. **products-v2.php v2**: sticky search, акордеон filter drawer, cascading filters
3. **products-v2.php v3**: ОБЩА КАРТИНА (store_id=0 = всички магазини) — 12 SQL queries преобразувани
4. **products-v2.php v4**: информативен бокс + поправка на цветни кръгчета + 2 нови филтра + цена нагоре

### Реални числа от тенант 7:
- Преди тестване: бяха показани 31 артикула в info-box
- Тих ще пусне SQL за реално разпределение на confidence_score
- Чака се резултат (SQL команда дадена в чата)

### НЕЗАВЪРШЕНО за utre/S144:

**INFO BOX V2 (3 нива вместо 2):**
- Текущо: пълни (≥80) / чакат (<80)
- Иска се: пълни (≥80) / частични (40-79) / минимални (<40)
- + обяснително съобщение: "AI не може да си върши работата без тази информация"

**ВЪПРОС за confidence_score изчислението:**
- Възможност 1: системата брои попълнените полета (12 × точки = 100)
- Възможност 2: статично при добавяне (wizard=100%, бързо=20%)
- Възможност 3: AI преценка
- Тих НЕ е избрал — нужно обсъждане

**INV-NUDGE (Закон §10):**
- Тих заяви: "info-box достатъчен" → inv-nudge да се МАХНЕ
- Но не сме го махнали още
- При следваща сесия → премахни inv-nudge button

### Standing Rule #49:
- "Преброяване" (last_counted_at) = периодична инвентаризация
- "Информация" (confidence_score) = data quality на артикулите
- Две различни концепции — НЕ се смесват

### КЛЮЧОВИ ЦИТАТИ от Тих (S143):
- "Това е огромен фундаментален пропуск" (за общата картина)
- "ОК САМО ЗАПИШИ ВСИЧКО В КОМПАСА" (за AI обучение стратегията)
- "НЕ ЗАБРАВЯЙ ЧЕ ЕДНОТО ОПИСАНИЕ ДЪЛГОТО Е ПЛАТЕНО" (за описанията)
- "НИКОЙ НЯМА ДА ПРАВИ ПОСТОЯННА ИНВЕНТАРИЗАЦИЯ!" (за inv-nudge)
- "ИНФО БОКСА ЗА МЕН Е ДОСТАТЪЧЕН!"
- "ТРЯБВА ДА ПИШЕ И КОЛКО СА ОБЩО АРТИКУЛИТЕ И КОЛКО СА С ПЪЛНА КОЛКО С ЧАСТИЧНА И КОЛКО С МИНИМАЛНА"

### S143 commits на main:
- 6f0b2fe — Step 2-3: rich search + filters
- 06aca3c — v2: sticky + акордеон
- 5cf3a9a — v3: ОБЩА КАРТИНА
- 3c833d1 — финална документация
- d33351c — fix Всички магазини default
- 627add8 — EOD Deep Research + 6 нови правила
- a90bd0d — v4: info-box + цветове + филтри

### S144 PRIORITIES (за следваща сесия):
1. INFO BOX V2 — 3 нива (пълна/частична/минимална)
2. Решение за confidence_score изчисление (Възможност 1/2/3)
...
## S143 — ДОПЪЛНЕНИЕ V1.1: ПЪЛНА СТРАТЕГИЯ ЗА СПЕСТЯВАНЕ + AI STUDIO ТЕСТ

### Какво беше пропуснато в v1.0:
Тих повдигна: "имаше разговор как да спестим пари — това влезе ли някъде?"

Реално:
- Ниво 1+2 на учене → беше в TOMORROW, но не в Source of Truth
- Ниво 3+4 → беше само в MASTER_COMPASS S143 секция
- Prompt caching, batch API, smart routing → споменати в Research, не централизирани
- AI Studio практически тест → беше само в чата, никъде не записан

### Какво добавено в v1.1:

#### `docs/AI_AUTOFILL_SOURCE_OF_TRUTH.md` v1.1:
Нова секция "СТРАТЕГИЯ ЗА СПЕСТЯВАНЕ НА ПАРИ (8 ТЕХНИКИ)":

1. **Нивелачно учене** (4 нива) — главна стратегия
2. **Prompt Caching** (90% намаление input токени)
3. **Batch API** (50% намаление за async)
4. **Smart Routing** (евтин за прости, скъп за сложни)
5. **Image Compression** преди обаждане (~30% input cost)
6. **Cross-Tenant Barcode Copying** (~10-15% намаление)
7. **Free Tier Лимити** (защита от експоненциален разход)
8. **Retry Логика без дубли** (защита от 2x billing)

Плюс прогноза: при 100 клиента с 8-те техники = -60% AI разходи
(€450 → €180/мес = €3,240/година спестено).

#### `TOMORROW_WIZARD_REDESIGN.md` v1.1:
Нова Стъпка 0 — "ПРЕДВАРИТЕЛЕН ТЕСТ В AI STUDIO":
- 1 час преди код
- 3 реални снимки от тенант 7
- Тест на Gemini Flash/Pro/Nano
- Реални числа за точност + цена + latency
- TODO: docs/AI_AUTOFILL_REAL_TESTS.md след теста

### Standing Rule #50:
- Преди всеки AI integration → AI Studio практически тест с реални
  снимки от тенанта. Спестява гадания и грешни архитектурни решения.

### Цитат на Тих (S143 EOD):
"ами нали имаше предложение за по подробно проучване което да ни спести
пари и иобщо имаше разговор как да спестим пари това влезе ли някъде
или го махнахме като въпрос?"


---

## S144 Logic Change — Native Back Button Guard (15.05.2026)

### Правило #51 (ЗАКОН за всеки готов модул):

**Задължителна имплементация на back-button guard в момента в който модулът се финализира.**

НЕ оставяме за след всички модули. Прави се per-module веднага щом модулът е "production ready".

### Защо

Native back бутон на телефона може да:
- Затвори wizard на стъпка 5 от 7 → загуба на 5 минути попълване
- Излезе от sale.php с пълна количка → загуба на продажба
- Прекъсне детайли drawer където редактираш → загуба на промяна
- Из стъпки на филтриране в списък → загуба на филтрите

### Pattern

Всеки модул след завършване:

1. `history.pushState({drawer: 'name'}, '', '#drawer')` при отваряне на drawer/wizard/filter
2. `window.addEventListener('popstate')` → handler:
   - Ако drawer отворен → затвори drawer (не страницата)
   - Ако wizard има данни → confirm "Ще загубиш данните?"
   - Ако филтър активен → reset филтър (не страница)
   - Иначе → пуска native back

### Сегашен статус

| Модул | Production ready | Back-guard статус |
|---|---|---|
| chat.php | ~90% | ⏳ Чака финализация → после guard |
| products.php | ~90% (rewrite в дизайн) | ⏳ Чака финализация → после guard |
| sale.php | ✅ done | ✅ Back-button забранен (sale.php е критичен) |
| inventory.php v3 | ~60% | ⏳ |
| Останали модули | <50% | ⏳ |
...
═══════════════════════════════════════════════════════════════
## S148 — ФИНАНСОВ МОДУЛ + TWO-PRODUCT ARCHITECTURE
═══════════════════════════════════════════════════════════════

### Status: Source-of-truth е `STATS_FINANCE_MODULE_BIBLE_v1.md` (12 000+ реда, ETAP 1-7)

### Two-Product Architecture (Закон §42)

**Един codebase, два продукта.** Дискриминация чрез `tenants.plan`:

| Product | plan | UI shell | Цена |
|---|---|---|---|
| **RunMyStore.AI** | start/pro/business | `life-board.php`, `products.php`, `stats.php`, `sale.php` | €19/49/109 |
| **Pocket CFO** | cfo | `cfo/home.php`, `cfo/records.php`, `cfo/analysis.php`, `cfo/goals.php` | €4.99 |

**Routing:** `index.php` → проверява `tenant->plan` → препраща към съответен UI.

**Споделен engine** (`/lib/`): `money-engine.php`, `voice-parser.php`, `photo-receipt-parser.php`, `ai-engine.php`, `bg-tax-engine.php`, `profession-templates.php` — един код, два продукта.

**ЗАБРАНЕНО:** `lib/money-engine.php` (RMS) + `lib/money-engine-cfo.php` (CFO) — дублирана логика. Bug fix трябва на 2 места.

**Commit prefixes:** `S15X ENGINE:` (shared) · `S15X RMS:` (RunMyStore-only) · `S15X CFO:` (Pocket CFO-only).

### s82-dash REVISED (S148, замества старата концепция)

**ОТМЕНЕНО:** s82-dash вече НЕ е "top product + low stock + dead capital + Поръчай бутон".

**НОВА концепция (от Bible §24):** s82-dash е **компактно финансово табло в life-board.php** — mini огледало на `cfo/home.php`:
- 1 голямо число (Operating Profit / Касов баланс)
- AI ротиращ insight slot (от ai_insights WHERE module='cfo')
- 3 quick action бутона: 🎤 Запиши · 📷 Снимка · 📊 Виж
- Tap → отваря `stats.php?tab=finance`

**Top product / low stock / dead capital → ОСТАВАТ в life-board.php но в ОТДЕЛНИ карти** (не в s82-dash).

**Mockup reference:** `mockups/P21_dash82_v2.html` (idе да дойде от design чат).

**Plan visibility:** START basic / PRO+BUSINESS full / FREE hidden / SELLER hidden (Пешо не вижда финанси).

### Финансов модул в RunMyStore (stats.php?tab=finance)

5 sub-таба, owner-only, redirect ако не owner:

| Sub-tab | Phase | Status |
|---|---|---|
| **12.1 Печалба** | **Phase B (beta)** | ✅ Включено в beta scope |
| 12.2 Cash | Phase 8 (post-beta) | "Скоро" badge |
| 12.3 Разходи | Phase 8 | "Скоро" badge |
| 12.4 Дължими | Phase 8 | "Скоро" badge |
| 12.5 Експорти | Phase 8 | "Скоро" badge |

**Phase B beta делiverable:** Печалба sub-tab с P&L breakdown, margin trend, top profit products, category margin, discount erosion. Confidence warning при `cost_at_sale < 100%`.

### БГ Tax Engine (universal, от Bible §44)

**Реализиран в `lib/bg-tax-engine.php`:**
- НПР калкулатор (25% свободни професии / 40% занаятчии / 60% земеделци)
- ДДС праг detector (€51 130 = 100K лв)
- ДДС chap 97a (за фрийлансъри/креатори ползващи EU services)
- Patent tax (фризьори/такси/маникюр)
- ППП detection (Еконт/Спиди наложен платеж)

**Полза за RMS owner-и:** автоматичен годишен данък estimate в stats.php?tab=finance.

### DB Schema additions (Phase B critical)

3 нови таблици (M-001, M-002, M-003 от Bible §6.1):
- `vat_rates` — БГ ставки 20%/9%/0%
- `z_reports` — дневен Z-отчет
- `store_balances` — opening/closing balance

Плюс универсалните (за двата продукта):
- `money_movements` — replaces sales+expenses+cash_drawer в единна таблица
- `categories` — универсална
- `vendor_aliases` + `vendor_aliases_universal` — self-learning vendor knowledge
- `bg_recurring_calendar` — 60+ БГ events
- `profession_templates` — 14 templates (95% БГ coverage)

### Pocket CFO (паралелен продукт)

**Не блокира RunMyStore beta.** Pocket CFO работи паралелно — отделна сесия / отделен CC chat.

**Целеви launch:** Phase B1 (юни 2026) — 3 седмици след RMS beta.

**Target market:** 14 професионални templates × ~340K самонаети в БГ = 95-97% coverage.

**60 AI теми (cfo_001-060):** anti-halucination, PHP-first parsing (намалява AI cost €0.10→€0.03/user/мес за 12 месеца).

### Документ референции

- `STATS_FINANCE_MODULE_BIBLE_v1.md` — пълна спецификация (single source of truth)
- `mockups/P19_stats_v2_overview.html` — Tab Преглед mockup (от design чат)
- `mockups/P21_dash82_v2.html` — s82-dash компакт mockup (предстои)
- `DESIGN_SYSTEM_v4.0_BICHROMATIC.md` §5.4 — Sacred Glass canon (важи за двата продукта)

### Implementation status (S148, 17.05.2026)

- ✅ Bible v1.3 финализирана (12 500 реда)
- ✅ Mockup 1 (P19_stats_v2_overview) committed
- ⏳ Phase B implementation (юни 2026)
- ⏳ s82-dash v2 mockup (P21_dash82_v2) — чакаме от design чат
- ⏳ Pocket CFO B1 (паралелна сесия)


═══════════════════════════════════════════════════════════════
## S148 — AI MARKUP ARCHITECTURE ПРЕРАЗПРЕДЕЛЕНИЕ (17.05.2026)
═══════════════════════════════════════════════════════════════

### Промяна на стратегия

**AI markup препоръка** НЕ е подходяща за **Section 1 на wizard "Добави артикул"**:

- Section 1 = бърз add от Пешо на момента — cost не се знае или се guess-ва
- AI markup върху guessed cost = безсмислено предложение
- Пешо забавя workflow с irrelevant AI suggestion

**Истинското място на AI markup = ДОСТАВКИ МОДУЛ (deliveries.php, Phase A).**

### ЗАКОН — AI Markup задължителен при доставка по документ

**Когато Пешо приема пратка с фактура/документ:**

1. **Input:** снимка на фактура / Excel upload / CSV от доставчик
2. **AI extraction:** Gemini Vision извлича `{product_name, cost_price, qty}` за всеки артикул
3. **AI markup ЗАДЪЛЖИТЕЛНО за всеки extracted item:**
   - Гледа `pricing_patterns` table (история на Пешо за тази категория)
   - Прилага formula от `AUTO_PRICING_DESIGN_LOGIC.md` (2.0× облекло, 2.5× обувки, 3.0× аксесоари)
   - Прилага ending psychology (.99, .95)
   - Връща `suggested_retail` + `markup_pct` + `confidence`
4. **UI:** batch confirmation screen
   - Списък 10-50 артикула накуп
   - Всеки ред: name + cost + **AI предлага €X.99** (editable)
   - Edge case: low confidence (<0.85) → marker "?"
   - Tap "Приеми всички" → batch INSERT
5. **Learning loop:** при одобрение → `pricing_patterns` се update-ва (per category × supplier)

### Защо това е killer feature

| Конкурент (Lightspeed/MicroInvest/Loyverse) | RunMyStore |
|---|---|
| Owner ръчно въвежда retail за всеки артикул при доставка | AI предлага retail за всичките 50 артикула накуп |
| 50 артикула × 30 sec = **25 минути работа** | 50 артикула × 1 tap = **30 секунди** |
| Грешки в ценообразуването = загубена печалба | Consistency базирана на собствената история на Пешо |
| Никаква pricing intelligence | Pricing patterns се учат с времето |

### Implementation locations (обновено)

- ❌ `wizard-v6.php Section 1` — НЕ подходящо (бърз manual add, cost често guess)
- ✅ `deliveries.php` — **ГЛАВНОТО МЯСТО, Phase A** (batch processing, реални cost)
- ⚠ `wizard edit drawer` — опционално (ако Пешо иска recalc на retail)
- ⚠ `bulk CSV import` — задължително (когато се имплементира, future)

### Code reuse

- `AUTO_PRICING_DESIGN_LOGIC.md` (568 реда) — универсален документ, не променя се
- `services/ai-markup.php` (sacred) — universal endpoint, ползва се от двата места:
  - `wizard-v6.php` (вече интегриран — ще оставим или махнем според Тих's решение)
  - `deliveries.php` (Phase A — главно място, batch loop)
- Bridge call: `services/wizard-bridge.php?action=ai_markup` — reuse-ва се

### Priority за Phase A

**Deliveries.php = HIGH PRIORITY** след wizard-v6.php beta release.

Изисквания:
1. `deliveries` DB table (вече зарезервирано в Bible)
2. Photo upload + Gemini Vision pipeline (вече има в `services/ai-vision.php`)
3. Excel/CSV parser (нов код, ~500 реда)
4. Batch AI markup loop (reuse `services/ai-markup.php`)
5. Batch confirmation UI (нов partial, ~400 реда)
6. INSERT в `stock_movements (type='delivery')` + `products` update

### Document references

- `AUTO_PRICING_DESIGN_LOGIC.md` — pricing formulas + confidence
- `STATS_FINANCE_MODULE_BIBLE_v1.md` §44 — БГ Tax Engine reference (за supplier EIK validation)
- `services/ai-markup.php` — sacred endpoint (не пипа)
- `services/ai-vision.php` — image-to-attributes (sacred, reuse за фактури)


═══════════════════════════════════════════════════════════════
## S149 — SHARED SCAN COMPONENT (voice + photo)
═══════════════════════════════════════════════════════════════

### Концепция

**Voice overlay** + **Photo receipt scanner** = един shared компонент, използван от **двата продукта** (Закон §42):

- **RunMyWallet (Pocket CFO):** scan лични касови бележки + voice records
- **RunMyStore:** scan фактури при доставка + voice add артикули

### Visual specs (mockups)

| Mockup | Файл | States | Описание |
|---|---|---|---|
| Voice Overlay | `mockups/P27_runmywallet_voice_overlay.html` | 5: idle / listening / processing / confirm / error | Voice → parser → AI parse → confirm card |
| Photo Receipt | `mockups/P28_runmywallet_photo_receipt.html` | 4: camera / captured / processing / confirm | Photo → Gemini Vision → structured data → confirm |
| Settings | `mockups/P26_runmywallet_settings.html` | static | Settings page (RunMyWallet config) |

### Implementation files

**Shared engine (lib/*) — един път писан, ползва се от двата продукта:**

```
lib/voice-parser.php          ← Whisper STT + Gemini parse
lib/photo-receipt-parser.php  ← Gemini Vision → structured JSON
lib/ner-anonymizer.php        ← PII removal от transcripts
```

**UI partials — споделени:**

```
partials/scan/voice-overlay.php       ← 5-state voice modal (от P27 mockup)
partials/scan/photo-receipt.php       ← 4-state camera flow (от P28 mockup)
partials/scan/scan-confirm-card.php   ← shared confirmation UI
```

### Usage в RunMyStore deliveries.php (Phase A)

```php
// deliveries.php — нов модул
require 'partials/scan/photo-receipt.php';  // фактура scan
require 'partials/scan/voice-overlay.php';  // voice add артикули

// Photo flow:
// 1. User snap фактура
// 2. PhotoReceiptParser → extract items
// 3. AIMarkupEngine batch loop (за всеки item)
// 4. Batch confirmation screen
// 5. INSERT в stock_movements + products

// Voice flow:
// 1. User натиска mic
// 2. VoiceParser → "получих 10 якета от Marina по 25 лева"
// 3. AI parse → {supplier:"Marina", item:"якета", qty:10, cost:12.78€}
// 4. Confirm card
// 5. INSERT
```

### Usage в Pocket CFO cfo/home.php

```php
// cfo/home.php — Quick action buttons
[🎤 Запиши] → отваря partials/scan/voice-overlay.php
[📷 Снимка] → отваря partials/scan/photo-receipt.php
[📊 Виж]    → отваря analysis.php
```

### Code reuse percentage

Според Закон §42:
- Scan UI partials: **100% shared** (Pocket CFO + RunMyStore deliveries)
- Backend parsers: **100% shared**
- Различни context wrappers (deliveries header vs CFO records header)

### Implementation order

1. **Първо имплементираме в Pocket CFO** (по-чист use case — лични харчове)
2. **После reuse в RunMyStore deliveries** (по-сложен — batch markup loop)
3. **Един commit засяга и двата продукта** (commit prefix: `S15X ENGINE:` за shared parts)
